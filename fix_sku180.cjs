...
const oldBlock = `  if (skuType === 'sku180') {
    const e = explode * 1.5;
    ...
    // Shift to roughly center it visually
    const groupX = -actualLen / 2;
    const groupY = 0;
    const groupZ = actualDepth / 2;
    ...
    return (
      <group position={[groupX, groupY, groupZ]}>
        ...
        <group position={[0, 0, 0]}>
          <CornerFitting position={[0, 0, 0]} side="right" rotation={[0, 0, Math.PI]} showLabel={showLabel} colorOption={colorOption} />
        </group>
