...
const Plug = ({ position, rotation = [0, 0, 0], showLabel, labelText = "In Cap", colorOption = COLORS['Raw grey'] }: { position: [number, number, number], rotation?: [number, number, number], showLabel?: boolean, labelText?: string, colorOption?: ColorOption }) => {
  return (
    <group position={position} rotation={rotation}>
...
const Rack = ({ length, height, wallDistance, explode, hasShelves = true, isFreestanding = false, colorOption = COLORS['Raw grey'], skuType = 'standard', woodColor = 'Natural Oak', tiers = 4, tubeType = 'round' }: { length: number, height: number, wallDistance: number, explode: number, hasShelves?: boolean, isFreestanding?: boolean, colorOption?: ColorOption, skuType?: 'standard' | 'sku777' | 'sku000' | 'sku100' | 'sku200' | 'sku102' | 'sku103' | 'sku104' | 'sku4210' | 'sku300' | 'sku105' | 'sku106' | 'sku107' | 'sku108' | 'sku109' | 'sku110' | 'sku111' | 'sku112' | 'sku113' | 'sku114' | 'sku115' | 'sku116' | 'sku117' | 'sku118' | 'sku119' | 'sku120' | 'sku121' | 'sku122' | 'sku123' | 'sku124' | 'sku125' | 'sku126' | 'sku127' | 'sku128' | 'sku129' | 'sku130' | 'sku131' | 'sku132' | 'sku133' | 'sku134' | 'sku135' | 'sku136' | 'sku137' | 'sku138' | 'sku140' | 'sku141' | 'sku142' | 'sku143' | 'sku144' | 'sku145' | 'sku146' | 'sku147' | 'sku148' | 'sku149' | 'sku150' | 'sku151' | 'sku152' | 'sku153' | 'sku154' | 'sku155' | 'sku156' | 'sku157' | 'sku158' | 'sku159' | 'sku160' | 'sku161' | 'sku162' | 'sku163' | 'sku164' | 'sku165' | 'sku166' | 'sku167' | 'sku168' | 'sku169' | 'sku170' | 'sku171' | 'sku172' | 'sku173' | 'sku174' | 'sku175' | 'sku176' | 'sku177' | 'sku178' | 'sku179' | 'sku180' | 'sku181' | 'sku182' | 'sku183' | 'sku184' | 'sku186' | 'sku187' | 'sku188' | 'sku189' | 'sku190' | 'sku191' | 'sku192' | 'sku193' | 'sku194' | 'sku888', woodColor?: string, tiers?: number, tubeType?: 'round' | 'square' }) => {
  ...
  if (skuType === 'sku144') {
    ...
    return (
      <group position={[-(2.2 + actualCutLength) / 2, 0, 0]}>
        ...
        {/* Standoff Wall Pipe */}
        <group position={[0, 0, 0]}>
          <Pipe start={[0, 0, zWall + 2.15]} end={[0, 0, zFront - 2.2]} showLabel={showLabel} colorOption={colorOption} />
        </group>
        ...
        <group position={[0, 0, zFront]}>
          <Pipe start={[2.2, 0, 0]} end={[2.2 + actualCutLength, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
        </group>
      ...
      <group position={[0, yOffset, 0]}>
        {xPositions.map((x, i) => {
          ...
          return (
            <group key={`post-${i}`}>
              {/* Floor Flange */}
              <group position={[x + dx, yFloor - e, zFront]}>
                <Flange position={[0, 0, 0]} rotation={[0, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>

              {/* Bottom Vert Pipe */}
              <group position={[x + dx, 0, zFront]}>
                <Pipe start={[0, yFloor + 2.15, 0]} end={[0, yMidTee - 2.2, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>

              {/* Mid Tee (points back to wall) */}
              <group position={[x + dx, yMidTee, zFront]}>
                {/* [0,0,0] points branch -Z backward */}
                <TFitting position={[0, 0, 0]} rotation={[0, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>

              {/* Mid Wall Pipe */}
              <group position={[x + dx, yMidTee, -e / 2]}>
                <Pipe start={[0, 0, zFront - 2.2]} end={[0, 0, zWall + 2.15]} showLabel={showLabel} colorOption={colorOption} />
              </group>

              {/* Mid Wall Flange */}
              <group position={[x + dx, yMidTee, zWall - e]}>
                <Flange position={[0, 0, 0]} rotation={[Math.PI / 2, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>
  ...
  if (skuType === 'sku149') {
    ...
    // Anchor top at Y=height/2 to feel centered on screen
    const spanY = actualCutHeight + 4.4;
    const topY = spanY / 2;
    const bottomY = -spanY / 2;

    const spanX = actualCutLength + 4.4;
    const xPositions = [-spanX / 2, spanX / 2];
    ...
    return (
      <group position={[0, 0, 0]}>
        {xPositions.map((x, i) => {
          ...
          return (
            <group key={`leg-${i}`}>
              {/* Wall Flange Base at Z=zWall, points +Z */}
              <group position={[x + dx, topY + e, zWall - e]}>
                <Flange position={[0, 0, 0]} rotation={[Math.PI / 2, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>

              {/* Depth Pipe (Wall Standoff) */}
              ...
              {/* Drop Pipe (from Top Elbow to Bottom Elbow) */}
              <group position={[x + dx, 0, zFront + e]}>
                <Pipe start={[0, topY - 2.2, 0]} end={[0, bottomY + 2.2, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>
        ...
        <group position={[0, bottomY - e, zFront + e]}>
          <Pipe start={[xPositions[0] + 2.2, 0, 0]} end={[xPositions[1] - 2.2, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
        </group>
      </group>
    );
    ...
    return (
      <group position={[0, height / 2, -wallZ / 2]}>
        ...
        {Array.from({ length: numMounts }).map((_, i) => {
          ...
          // Explode along actual connection paths
          const pushXY = e * 0.5;
  ...
  if (skuType === 'sku181') {
    ...
    return (
      <group position={[0, groupY, actualStem / 2]}>
        ...
        {/* Bottom In Cap */}
        <group position={[0, -e, 0]}>
          <EndCap position={[0, -2.2, 0]} rotation={[Math.PI, 0, 0]} labelText="In cap" showLabel={showLabel} colorOption={colorOption} />
        </group>
        ...
        {/* Top End Cap */}
        <group position={[0, e * 2, 0]}>
          <EndCap position={[0, actualHeight + 2.2, 0]} rotation={[0, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
        </group>
  ...
  if (skuType === 'sku184') {
    ...
    return (
      <group position={[0, -2, 2.9]}>
        ...
        <group position={[-6.8 - e * 2, 3.4 + e, 0]}>
          <HexNipple position={[0, 0, 0]} rotation={[0, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
        </group>
        <group position={[-6.8 - e * 2, 5.2 + e * 2, 0]}>
  ...
  if (skuType === 'sku189') {
        ...
        <group key={type}>
          ...
          {hasShelves ? (
            <>
              ...
              <group position={[sideX, e * 2.5, 0]}>
                 <HexNipple position={[x, vertCut + 3.2, 0]} rotation={[0, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>

              {/* Floor Flange directly under the shelf */}
              <group position={[sideX, e * 3, 0]}>
                <Flange position={[x, vertCut + 4.2, 0]} rotation={[0, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>
            </>
          ) : (
             <group position={[sideX, e * 2, 0]}>
    ...
    return (
      <group position={[0, Math.max(0, 100 - vertCut), 0]}>
        ...
        {hasShelves && (
          <group position={[0, e * 3, 0]}>
            <Shelf position={[0, vertCut + 6.1, 0]} length={length + 10} depth={23} woodColor={woodColor} />
          </group>
        )}
      </group>
    );
  ...
  if (skuType === 'sku194') {
    ...
    const numMounts = isLong ? 3 : 2;
    
    const mounts = [];
    if (isLong) {
      mounts.push({ x: -lCut / 2, type: 'end' });
      mounts.push({ x: 0, type: 'mid' });
      mounts.push({ x: lCut / 2, type: 'end' });
    } else {
      mounts.push({ x: -lCut / 2, type: 'end' });
      mounts.push({ x: lCut / 2, type: 'end' });
    ...
    return (
      <group position={[0, -yRail, -zWall / 2]}>
        
        {mounts.map((mount, index) => {
          const signX = mount.x === 0 ? 0 : Math.sign(mount.x);
          const localE = e * signX;
        ...
        {isLong ? (
           <>
              <group position={[-e * 0.5, yRail, 0]}>
                  <Pipe start={[-lCut/2 + 2.2, 0, 0]} end={[-2.2, 0, 0]} showLabel={showLabel} colorOption={colorOption} overrideLabel={`${((lCut - 3) / 2).toFixed(1)} cm`} />
              </group>
              <group position={[e * 0.5, yRail, 0]}>
                  <Pipe start={[2.2, 0, 0]} end={[lCut/2 - 2.2, 0, 0]} showLabel={showLabel} colorOption={colorOption} overrideLabel={`${((lCut - 3) / 2).toFixed(1)} cm`} />
              </group>
           </>
        ) : (
           <group position={[0, yRail, 0]}>
               <Pipe start={[-lCut/2 + 2.2, 0, 0]} end={[lCut/2 - 2.2, 0, 0]} showLabel={showLabel} colorOption={colorOption} overrideLabel={`${lCut} cm`} />
           </group>
        )}
      </group>
    );
  ...
  if (skuType === 'sku191') {
    ...
    const buildSupport191 = (x, type) => {
      const sideX = type === 'left' ? -e : e;
      const tRot = type === 'left' ? [0, 0, 0] : [0, Math.PI, Math.PI];
        ...
        <group key={type}>
          {hasShelves && (
            <group>
              ...
              {/* TOP ELBOW */}
              <group position={[sideX, e*5, 0]}>
                <Elbow position={[x, topDrop, 0]} rotation={[0, Math.PI, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>
              ...
              <group position={[sideX, e*3, 0]}>
                <Flange position={[x, 1.2, 0]} rotation={[Math.PI, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>
            </group>
          )}

          {/* BOTTOM MOUNT: T-FITTING (or ELBOW if no shelves) */}
          <group position={[sideX, 0, -e]}>
          ...
          {hasShelves ? (
            <group>
              {/* TEES and UNDER SHELF FLANGE */}
              <group position={[sideX, 0, 0]}>
                 {/* Middle TFitting (Branch -Z, Run Y). Run connects up and down. */}
                 <TFitting position={[x, -2.5, 0]} rotation={[0, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>
              <group position={[sideX, e, 0]}>
                 <HexNipple position={[x, -0.3, 0]} rotation={[0, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>
              <group position={[sideX, e*2, 0]}>
                 <Flange position={[x, 0.7, 0]} rotation={[0, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>
              {/* VERTICAL DROP TO BAR */}
              <group position={[sideX, -e, 0]}>
                 <Pipe start={[x, -4.7, 0]} end={[x, -bottomDrop + 2.2, 0]} showLabel={showLabel} colorOption={colorOption} overrideLabel={`${bottomDrop} cm`} />
              </group>
            </group>
          ) : (
            <group>
              {/* STANDIN FOR NOTHING (NO TEE, 90 ELBOW INSTEAD!) */}
              <group position={[sideX, 0, 0]}>
                 <Elbow position={[x, -2.5, 0]} rotation={[Math.PI, 0, 0]} showLabel={showLabel} colorOption={colorOption} />
              </group>
              {/* VERTICAL DROP TO BAR */}
              <group position={[sideX, -e, 0]}>
                 <Pipe start={[x, -4.7, 0]} end={[x, -bottomDrop + 2.2, 0]} showLabel={showLabel} colorOption={colorOption} overrideLabel={`${bottomDrop} cm`} />
          ...
          {/* BOTTOM CORNERS INWARD */}
          <group position={[sideX, -e*2, 0]}>
             <Elbow position={[x, -bottomDrop, 0]} rotation={[0, type === 'left' ? Math.PI/2 : -Math.PI/2, Math.PI]} showLabel={showLabel} colorOption={colorOption} />
          </group>
        </group>
      );
    };
    ...
    return (
      <group position={[0, hasShelves ? -(topDrop/2) : (bottomDrop/2), zWall]}>
        {buildSupport191(leftX, 'left')}
        {buildSupport191(rightX, 'right')}

        {/* Top Horizontal Clothing Bar */}
        {horizCut > 120 ? (
          <>
            <group position={[-explode * 0.75, -e * 2, 0]}>
              <Pipe start={[leftX + 2.2, -bottomDrop, 0]} end={[0, -bottomDrop, 0]} showLabel={showLabel} colorOption={colorOption} overrideLabel={`${(lCut/2).toFixed(1)} cm`} />
            </group>
            <group position={[0, -e * 2, 0]}>
              <Coupling position={[0, -bottomDrop, 0]} rotation={[0, 0, Math.PI / 2]} showLabel={showLabel} colorOption={colorOption} />
            </group>
            <group position={[explode * 0.75, -e * 2, 0]}>
              <Pipe start={[0, -bottomDrop, 0]} end={[rightX - 2.2, -bottomDrop, 0]} showLabel={showLabel} colorOption={colorOption} overrideLabel={`${(lCut/2).toFixed(1)} cm`} />
            </group>
          </>
        ) : (
          <group position={[0, -e * 2, 0]}>
            <Pipe start={[leftX + 2.2, -bottomDrop, 0]} end={[rightX - 2.2, -bottomDrop, 0]} showLabel={showLabel} colorOption={colorOption} overrideLabel={`${lCut} cm`} />
          </group>
        )}

        {/* Optional Shelf in Middle */}
        {hasShelves && (
          <group position={[0, e * 2.5, 0]}>
            <Shelf position={[0, 2.7, 0]} length={length + 10} depth={23} woodColor={woodColor} />
          </group>
        )}
      </group>
    );
...
const Scene = React.memo(({ length, height, wallDistance, explode, hasShelves, isFreestanding, colorOption, skuType, woodColor, cameraState, tiers = 4, tubeType = 'round' }: { length: number, height: number, wallDistance: number, explode: number, hasShelves: boolean, isFreestanding: boolean, colorOption: ColorOption, skuType: 'standard' | 'sku777' | 'sku000' | 'sku100' | 'sku200' | 'sku102' | 'sku103' | 'sku104' | 'sku4210' | 'sku300' | 'sku105' | 'sku106' | 'sku107' | 'sku108' | 'sku109' | 'sku110' | 'sku111' | 'sku112' | 'sku113' | 'sku114' | 'sku115' | 'sku116' | 'sku117' | 'sku118' | 'sku119' | 'sku120' | 'sku121' | 'sku122' | 'sku123' | 'sku124' | 'sku125' | 'sku126' | 'sku127' | 'sku128' | 'sku129' | 'sku130' | 'sku131' | 'sku132' | 'sku133' | 'sku134' | 'sku135' | 'sku136' | 'sku137' | 'sku138' | 'sku140' | 'sku141' | 'sku142' | 'sku143' | 'sku144' | 'sku145' | 'sku146' | 'sku147' | 'sku148' | 'sku149' | 'sku150' | 'sku151' | 'sku152' | 'sku153' | 'sku154' | 'sku155' | 'sku156' | 'sku157' | 'sku158' | 'sku159' | 'sku160' | 'sku161' | 'sku162' | 'sku163' | 'sku164' | 'sku165' | 'sku166' | 'sku167' | 'sku168' | 'sku169' | 'sku170' | 'sku171' | 'sku172' | 'sku173' | 'sku174' | 'sku175' | 'sku176' | 'sku177' | 'sku178' | 'sku179' | 'sku180' | 'sku181' | 'sku182' | 'sku183' | 'sku184' | 'sku186' | 'sku187' | 'sku188' | 'sku189' | 'sku190' | 'sku191' | 'sku192' | 'sku193' | 'sku194' | 'sku888', woodColor: string, cameraState?: any, tiers?: number, tubeType?: 'round' | 'square' }) => {
  return (
    <>
...
export default function App() {
  ...
  const [skuType, setSkuType] = useState<'standard' | 'sku777' | 'sku000' | 'sku100' | 'sku200' | 'sku102' | 'sku103' | 'sku104' | 'sku4210' | 'sku300' | 'sku105' | 'sku106' | 'sku107' | 'sku108' | 'sku109' | 'sku110' | 'sku111' | 'sku112' | 'sku113' | 'sku114' | 'sku115' | 'sku116' | 'sku117' | 'sku118' | 'sku119' | 'sku120' | 'sku121' | 'sku122' | 'sku123' | 'sku124' | 'sku125' | 'sku126' | 'sku127' | 'sku128' | 'sku129' | 'sku130' | 'sku131' | 'sku132' | 'sku133' | 'sku134' | 'sku135' | 'sku136' | 'sku137' | 'sku138' | 'sku140' | 'sku141' | 'sku142' | 'sku143' | 'sku144' | 'sku145' | 'sku146' | 'sku147' | 'sku148' | 'sku149' | 'sku150' | 'sku151' | 'sku152' | 'sku153' | 'sku154' | 'sku155' | 'sku156' | 'sku157' | 'sku158' | 'sku159' | 'sku160' | 'sku161' | 'sku162' | 'sku163' | 'sku164' | 'sku165' | 'sku166' | 'sku167' | 'sku168' | 'sku169' | 'sku170' | 'sku171' | 'sku172' | 'sku173' | 'sku174' | 'sku175' | 'sku176' | 'sku177' | 'sku178' | 'sku179' | 'sku180' | 'sku181' | 'sku182' | 'sku183' | 'sku184' | 'sku186' | 'sku187' | 'sku188' | 'sku189' | 'sku190' | 'sku191' | 'sku192' | 'sku193' | 'sku194' | 'sku888'>('standard');
  ...
  const [savedSKUs, setSavedSKUs] = useState<SavedSKU[]>(() => {
    ...
    const default194: SavedSKU = { name: 'SKU 194', length: 100, height: 0, wallDistance: 30, hasShelves: false, isFreestanding: false, colorName: 'Black', skuType: 'sku194' };
        ...
        const default181: SavedSKU = { name: 'SKU 181', length: 30, height: 30, wallDistance: 5, hasShelves: false, isFreestanding: false, colorName: 'Black', skuType: 'sku181' };
        const default180: SavedSKU = { name: 'SKU 180', length: 60, height: 60, wallDistance: 60, hasShelves: false, isFreestanding: false, colorName: 'Black', skuType: 'sku180' };
    const default179: SavedSKU = { name: 'SKU 179', length: 120, height: 0, wallDistance: 30, hasShelves: false, isFreestanding: false, colorName: 'Black', skuType: 'sku179' };
          ...
          <div className="lg:absolute lg:top-6 lg:left-6 z-10 bg-white/90 lg:backdrop-blur-xl lg:rounded-[32px] lg:shadow-2xl w-full lg:w-[340px] border-b lg:border border-gray-200 lg:border-white/40 flex flex-col lg:max-h-[calc(100vh-3rem)] overflow-visible lg:overflow-hidden order-2 lg:order-none shrink-0 rounded-none shadow-sm">
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar space-y-8">
              ...
              <section className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                ...
                <div className="space-y-6">
                  {skuType !== 'sku114' && skuType !== 'sku120' && skuType !== 'sku122' && skuType !== 'sku152' && skuType !== 'sku181' && (
                    <div>
                  ...
                  {((skuType as string) === 'standard' || skuType === 'sku000' || skuType === 'sku100' || skuType === 'sku4210' || skuType === 'sku105' || skuType === 'sku114' || skuType === 'sku116' || skuType === 'sku112' || skuType === 'sku111' || skuType === 'sku113' || skuType === 'sku117' || skuType === 'sku118' || skuType === 'sku119' || skuType === 'sku120' || skuType === 'sku121' || skuType === 'sku122' || skuType === 'sku123' || skuType === 'sku126' || skuType === 'sku127' || skuType === 'sku128' || skuType === 'sku130' || skuType === 'sku131' || skuType === 'sku132' || skuType === 'sku133' || skuType === 'sku135' || skuType === 'sku145' || skuType === 'sku150' || skuType === 'sku180' || skuType === 'sku181' || skuType === 'sku182' || skuType === 'sku183' || skuType === 'sku184' || skuType === 'sku186' || skuType === 'sku187' || skuType === 'sku188' || skuType === 'sku189' || skuType === 'sku190' || skuType === 'sku191' || skuType === 'sku192' || skuType === 'sku193' || skuType === 'sku194') && (
                    <div>
                      ...
                      <input
                        type="range"
                        min={skuType === 'sku119' || skuType === 'sku180' || skuType === 'sku181' || (skuType === 'sku182' || skuType === 'sku183' || skuType === 'sku184' || skuType === 'sku186' || skuType === 'sku187' || skuType === 'sku188' || skuType === 'sku189' || skuType === 'sku190' || skuType === 'sku191' || skuType === 'sku192' || skuType === 'sku193' || skuType === 'sku194') ? 5 : skuType === 'sku116' || skuType === 'sku133' ? 5 : skuType === 'sku122' ? 2 : skuType === 'sku128' ? 15 : 20} max={skuType === 'sku122' ? 10 : skuType === 'sku133' ? 30 : skuType === 'sku128' ? 100 : 250} step={skuType === 'sku122' || skuType === 'sku133' ? 1 : 5}
                        value={height}
