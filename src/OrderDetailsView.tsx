...
export const getCutlistItems = (config: any): CutlistItem[] => {
  ...
  const getTotalExtraCouplings = () => {
    ...
    // Floor-to-Wall with Shelf Option (Identical pipes to 4210 but 4210 misses T-Fittings in cutlist lol)
  ...
  if (skuType === 'sku777') {
    ...
    if (hasShelves) {
        addShelves(length, wallDistance, 1);
        addScrews(quantity * 4);
    }
  } else if (skuType === 'sku300') {
    const numPipes = Math.ceil(length / 120);
    const numMounts = numPipes + 1;
  ...
  if (skuType === 'sku777') {
    ...
    addPipes(15, 2, 'p-outboard-extension');
    
    const centerSpan = Math.max(0, length - 41.2);
  ...
  } else if (skuType === 'sku180') {
    addPipes(Math.max(0, height - 5), 1, 'p-vert-drop');
    addPipes(Math.max(0, length - 5), 1, 'p-horiz-len');
    addPipes(Math.max(0, wallDistance - 5), 1, 'p-horiz-depth');
    
    addFitting('f-wall-flanges', 'Wall Flanges', quantity * 3);
    addFitting('f-3way-corner-elbows', '3-Way Corner Elbows', quantity * 1);
    
    const dropCouplings = getExtraCouplings(Math.max(0, height - 5), 1);
    const lenCouplings = getExtraCouplings(Math.max(0, length - 5), 1);
    const depthCouplings = getExtraCouplings(Math.max(0, wallDistance - 5), 1);
    const totalCouplings = dropCouplings + lenCouplings + depthCouplings;
    if (totalCouplings > 0) {
      addFitting('f-couplings', 'Couplings', quantity * totalCouplings);
    }
  } else if (skuType === 'sku181') {
  ...
  } else if (skuType === 'sku188') {
    // Ceiling-to-Wall Rack: 2x Ceiling Flanges, 1x Wall Flange, 2x Vertical Pipes, 2x Horizontal Pipes, 1x T-Fitting, 1x 90-degree Elbow
    const horizCut = Math.max(0, length - 5);
    const vertCut = Math.max(0, height - 5);
    const pLen = horizCut / 2;

    addPipes(vertCut, 2, 'p-vert');
    addPipes(pLen, 2, 'p-horiz');

    addFitting('f-wall-flanges', 'Wall/Ceiling Flanges', quantity * 3);
    addFitting('f-t-fittings', 'T-Fittings', quantity * 1);
    addFitting('f-90-elbows', '90° Elbows', quantity * 1);

    const pipeCouplings = getExtraCouplings(vertCut, 2) + getExtraCouplings(pLen, 2);
    if (pipeCouplings > 0) {
      addFitting('f-couplings', 'Couplings', quantity * pipeCouplings);
    }
